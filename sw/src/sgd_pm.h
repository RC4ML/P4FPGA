...
private: 
	...
	uint32_t     *a_bitweaving_distribute;
	...
	uint32_t          *bi_fpga;	        // Integer version of dr_bi, accessed by FPGA.
...
public:
	...
	void a_perform_bitweaving_fpga(int worker_index);
	void b_copy_to_fpga(void);



	// Provide: float x[numFeatures]
	void bitFSGD(fpga::XDMAController* controller,uint32_t number_of_bits, uint32_t numberOfIterations, uint32_t mini_batch_size, uint32_t stepSize, int binarize_b, float b_toBinarizeTo,int node_index,int server_en); 
