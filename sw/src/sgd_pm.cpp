...
void zipml_sgd_pm::bitFSGD(fpga::XDMAController* controller,uint32_t number_of_bits, uint32_t numberOfIterations, uint32_t mini_batch_size, uint32_t stepSize, int binarize_b, float b_toBinarizeTo,int node_index,int server_en) 
{
   ...
   if(node_index == 0){
	...
	worker_ip[0] = 0xc0a8bd01;
	worker_ip[1] = 0xc0a8bd02;
	worker_ip[2] = 0xc0a8bd03;
	worker_ip[3] = 0xc0a8bd04;
...
// }
	...
	cout << "dma_wr_pkg_cnt: " << hex << controller ->readReg(524)<< endl;
	cout << "dma_wr_length_cnt: " << hex << controller ->readReg(528)<< endl;


	cout << "dma_wr_back_cmd_cnt: " << hex << controller ->readReg(896)<< endl;
	cout << "dma_wr_back_data_cnt: " << hex << controller ->readReg(897)<< endl;
	cout << "dma_wr_back_state: " << hex << controller ->readReg(898)<< endl;
